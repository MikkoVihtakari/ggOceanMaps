...
basemap <- function(
  ...
  if (is.null(bathy.style)) {
    bathy.style <- ifelse(
      !is.null(getOption("ggOceanMaps.bathy.style")),
  ...
  if (bathymetry & !is.null(shapefiles) & inherits(shapefiles, "list")) {
    if (!is.null(shapefiles$bathy)) {
      if (
        grepl("raster", bathy.type) &
          !inherits(shapefiles$bathy$raster, c("stars", "stars_proxy"))
      ) {
        if (
          inherits(
            shapefiles$bathy$raster,
            c("sf", "sfc", "SpatialPolygonsDataFrame", "SpatialPolygons")
          )
        ) {
          msg <- paste0(
          ...
          bathy.type <- gsub("_blues$|_grays$", "", names(bathy_cmd))
          bathy.type <- ifelse(
            grepl("raster_binned|raster_continuous", bathy.type),
            bathy.type,
            "vector"
          )
        }
      ...
      stop(
        "shapefiles = list(..., bathy) is required when using custom shapefiles with bathymetry = TRUE"
      )
    }
  }
  ...
  if (is.null(data) & is.null(limits) & is.null(shapefiles)) {
    stop(
      "One or several of the arguments limits, data and shapefiles is required."
    )
  }
  if (!is.logical(legends) | !length(legends) %in% 1:2) {
    stop(
      "'legends' argument has to be a logical vector of length 1 or 2. Read the explantion for the argument in ?basemap"
    )
  }
  if (!is.null(crs) & rotate) {
    rotate <- FALSE
    message(
      "The rotate argument cannot be used with custom crs. Turning rotate to FALSE."
    )
  }
  ...
  if (!is.null(limits)) {
    if (!length(limits) %in% c(1, 4)) {
      stop(
        "The limits argument has to be numeric vector of length 1 or 4. See Arguments."
      )
    }
    if (!inherits(limits, c("numeric", "integer"))) {
      stop("The limits argument has to be a numeric vector. See Arguments.")
    }
  }
  ...
  if (bathymetry & !is.null(X$shapefiles$bathy)) {
    ...
    if (bathy.type == "raster_binned") {
      layers <- paste(
        ...
        ifelse(
          bathy_color == "blues",
          map_cmd("bathy_rbb_scale"),
          map_cmd("bathy_rbg_scale")
        ),
        sep = " + "
      )
      ...
      layers <- paste(
        ...
        ifelse(
          bathy_color == "blues",
          map_cmd("bathy_rcb_scale"),
          map_cmd("bathy_rcg_scale")
        ),
        sep = " + "
      )
    } else {
