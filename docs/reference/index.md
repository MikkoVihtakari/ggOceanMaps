...
functions that work almost similarly to [`ggplot()` as a
...
`list(bathy =)`. High-resolution bathymetry can also be downloaded on
demand from a Web Coverage Service (WCS) endpoint using
`wcs_bathymetry`. The `clip_shapefile` function can be used to crop
already existing shapefiles.
...
- [`is_decimal_limit()`](https://mikkovihtakari.github.io/ggOceanMaps/reference/is_decimal_limit.md)
...
- [`round_any()`](https://mikkovihtakari.github.io/ggOceanMaps/reference/round_any.md)
...
- [`select_element()`](https://mikkovihtakari.github.io/ggOceanMaps/reference/select_element.md)
