...
output: 
  rmarkdown::html_vignette:
    toc_depth: 6
vignette: >
  %\VignetteIndexEntry{New features in ggOceanMaps version 2}
...
# The user-raster ("rub") examples only run when a local GEBCO/ETOPO grid is
# available (set GGOCEANMAPS_USERPATH); otherwise they are skipped at build time.
...
```{r, fig.cap = "**Figure:** Depth of data frame coordinates extracted from the raster_continuous ETOPO 60 arc-second dataset. Grey (NA) indicates coordinates on land."}
qmap(dt, color = depth) +
...
We **can now also plot maps crossing the anti-meridian**. Use the `rotate` argument to turn on the crossing:
...
```{r, eval = have_gebco, fig.cap = "**Figure:** Lofoten plotted using three CRS: a) 4326, i.e. decimal degrees, b) 3995, i.e. Arctic stereographic and c) 32633, i.e. UTM zone 33N."}
cowplot::plot_grid(
