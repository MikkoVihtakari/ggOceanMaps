...
vignette: >
  %\VignetteIndexEntry{Pre-made shapefiles}
...
This document records how the pre-made shapefiles bundled with — or downloaded
by — ggOceanMaps are produced, so the maps can be regenerated or adapted. The
rendered maps are shown in the
[Premade maps article](https://mikkovihtakari.github.io/ggOceanMaps/articles/premade-maps.html).

The low-resolution global shapefiles (`dd_land`, `dd_rbathy`) ship with the
package. Higher-resolution regional sets are stored in the
...
> The code below is shown for reference and is **not run** when this article is
> built. It uses the current `sf`/`stars` toolchain. Earlier versions used
> `sp`/`rgeos`; those packages are retired and the equivalents here are
> `sf::st_transform()` (reproject) and `sf::st_make_valid()` (fix geometries).

# Setup

Download the source datasets (see [Data sources](#data-sources)) and point the
path objects at the folders on your computer.
...
Coordinate reference systems are taken straight from `shapefile_list()`:
...
Bathymetry — both the binned raster shipped with the package and a continuous
version:
...
Land — Natural Earth land + minor islands. Natural Earth polygons require the
s2 engine to be turned off while clipping:
...
Land and glaciers — reproject the global layers with `sf::st_transform()` and
...
The Svalbard land, glacier, and bathymetry layers (EPSG:32633) follow the same
pattern: clip the source rasters/polygons to the Svalbard extent
(`shapefile_list("Svalbard")$limits`), reproject with `sf::st_transform()`, and
`vector_bathymetry()` the binned raster. They originate from the
[PlotSvalbard](https://github.com/MikkoVihtakari/PlotSvalbard) package.

<!--
Archived build recipes (kept for reference; the maps are not currently in
shapefile_list()). They may return once rebuilt with the current sf/stars
pipeline. The old recipes relied on retired sp/rgeos functions
(sp::spTransform -> sf::st_transform; rgeos::gBuffer(width = 0) ->
sf::st_make_valid) and the removed `aggregation.factor` argument of
raster_bathymetry() (use `downsample` instead).
...
barentssea_rbathy <- raster_bathymetry(
  bathy = file.path(gebcoPath, "GEBCO_2021.nc"),
  depths = NULL, proj.out = "EPSG:32636",
  boundary = bound.limits, warp = TRUE
)
# land/glacier: clip_shapefile(...) |> st_transform("EPSG:32636") |> st_make_valid()

## High-resolution Arctic (GEBCO, EPSG:3995)

rb <- raster_bathymetry(
  bathy = file.path(gebcoPath, "GEBCO_2021.nc"),
  depths = c(50, 100, 200, 300, 400, 500, 750, 1000, 1500, 2000,
             3000, 4000, 5000, 6000, 10000),
  proj.out = arcticCRS, boundary = c(-180, 180, 10, 90)
)
