...
- **Bbox size is capped** at 50°² by default via `max_area_deg2`; raise
...
Use `clip_shapefile()` to crop any sf polygon layer to a bounding box:
...
The pattern is: load a velocity NetCDF, thin the grid to a drawable density,
transform coordinates if on a projected map, then draw arrows with
`geom_segment()`.
...
**On a projected (polar) map** the coordinates must be transformed before
plotting (the map axes are in metres, not degrees):
